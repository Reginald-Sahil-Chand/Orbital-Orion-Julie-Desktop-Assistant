...
directory_io_handler.py:
========================
This file contains DirectoryIoHandler class, responsible to handle directory operations.
...
class DirectoryIoHandler:
    ...
    def create_directory(self, directory_path: str) -> None:
        ...
        try:
            # Create directory given the directory path and names.
            # If the given directory name does not resemble any existing,
            # Parent directory path than a new directory will be created in the root,
            # Folder where this program is running followed by the sub directories if any.
            # * Note: Here the name of the directory equals the directory_path.
            makedirs(name=directory_path, exist_ok=True)

        except DirectoryException as err:
            self._log_handler.create_log(
                log_type="error",
                log_message=f"A error related to directory has occurred. {err}")
