...
file_operation.py:
==================
This file contains FileOperation class, responsible to handle file operations.
...
from src.app.utility.helper._module.validation.custom_value_and_type_validation\
    .custom_value_and_type_validation import \
        (validate_file_contents, validate_if_type_does_not_match, validate_if_value_is_empty, )


@dataclass
class FileOperation(AbstractFileOperation):
    """Class to handle file operations, such as file creation."""

    # Instantiate LogHandler.
    log_handler: LogHandler = field(default_factory=LogHandler)

    # Instantiate DirectoryHandler.
    directory_operation: DirectoryOperation = field(default_factory=DirectoryOperation)
